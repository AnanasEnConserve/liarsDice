...
//                    print("Matching \(result.p.name) with utility \(result.u)")
    ...
    */
    ...
    /**
    Alternative reset. The difference between this and the reset function is that with resetAlt, buffers and dm chunks are not flushed. This is needed as the model needs to remember buffer contents/dm chunks, when actual model for playing game is loaded. With each new load(check load function in model.swift file), model resets everthing so we need to prevent that.
    */
